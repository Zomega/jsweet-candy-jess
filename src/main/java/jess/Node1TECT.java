...
class Node1TECT extends Node1 {
  Node1TECT(int paramInt1, int paramInt2, int paramInt3, int paramInt4, Value paramValue, Rete paramRete) {
  ...
  boolean callNode(Token paramToken, int paramInt) throws ReteException {
    ...
    boolean bool = (valueVector.get(0).atomValue() == this.R1 && valueVector.get(1).descriptorValue() == this.R2) ? true : false;
    if (bool)
