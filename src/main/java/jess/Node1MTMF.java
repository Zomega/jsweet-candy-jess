...
class Node1MTMF extends Node1 {
  ...
  boolean callNode(Token paramToken, int paramInt) throws ReteException {
    ...
    int i = this.R1;
    int j = this.R2;
    if (valueVector1.get(i).equals(Funcall.NIL())) return false;
    ...
    for (byte b = 0; b < k - j + 1; b++) {
      ...
      int m;
      for (m = 0; m < j; m++) valueVector3.add(valueVector2.get(m));
      ValueVector valueVector4 = new ValueVector(i1);
      valueVector3.add(new Value(valueVector4, 512));
      for (m = j; m < i1 + j; m++) valueVector4.add(valueVector2.get(m));
      for (m = j + i1; m < n - 1 + i1; m++) valueVector3.add(valueVector2.get(m));
