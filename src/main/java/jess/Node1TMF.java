...
class Node1TMF extends Node1 {
  Node1TMF(int paramInt1, int paramInt2, int paramInt3, int paramInt4, Value paramValue, Rete paramRete) {
  ...
  boolean callNode(Token paramToken, int paramInt) throws ReteException {
    ...
    for (byte b = 0; b < i - this.R1 + 1; b++) {
      ...
      int j;
      for (j = 0; j < this.R1; j++)
        valueVector1.add(valueVector.get(j));
      ValueVector valueVector2 = new ValueVector(m);
      valueVector1.add(new Value(valueVector2, 512));
      for (j = this.R1; j < m + this.R1; j++)
        valueVector2.add(valueVector.get(j));
      for (j = this.R1 + m; j < k - 1 + m; j++)
        valueVector1.add(valueVector.get(j));
